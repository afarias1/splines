...
png_reader::png_reader(const string& fname){
  ...
  if (!m_png_ptr){
    fclose(fp);
    throw runtime_error("[read_png_file] png_create_read_struct failed\n");
  }

  m_info_ptr = png_create_info_struct(m_png_ptr);
  if (!m_info_ptr){
    fclose(fp);
    throw runtime_error("[read_png_file] png_create_info_struct failed\n");
  }
...
RGBColor png_reader::operator()(int row, int col){
 ...
 png_byte* pix_ptr = &row_buf[col*3]; //Assumes RGB 3 bytes per pixel
